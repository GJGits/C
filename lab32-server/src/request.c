        ...
        if (readRequest(connSock, request) == 0) {
            ...
            if(extractFileName(request, fileName) == 0) {
                ...
                if(checkFile(fileName) == 0) {
                    ...
                    printf("Slave[%d][error]: " ANSI_COLOR_RED "FILE %s DOES NOT EXIST!" ANSI_COLOR_RESET "\n", slaveNum, fileName);
                    free(request);
                    free(fileName);
                ...
                printf("Slave[%d][error]: " ANSI_COLOR_RED "CANNOT EXTRACT FILE NAME FROM REQUEST (%s)" ANSI_COLOR_RESET "\n", slaveNum, request);
                free(request);
                if(send(connSock, errorMessage, strlen(errorMessage), MSG_NOSIGNAL) == -1)
                    n = -1;
            }
            ...
            printf("Slave[%d][error]: " ANSI_COLOR_RED "WRONG REQUEST" ANSI_COLOR_RESET "\n", slaveNum);
            free(request);
            if(send(connSock, errorMessage, strlen(errorMessage), MSG_NOSIGNAL) == -1)
                return;
        }
...
void sendFile(int connSock, char *fileName) {
    ...
    if (fp != NULL) {
        ...
        while (progress < 100) {
          ...
          progress = ((double) bytes_sent / (double) ntohl(fileInfo[0])) * 100;
          printf("\rSlave[%d][sending]: " ANSI_COLOR_CYAN "SENDING OF %s, %d [%d%%]" ANSI_COLOR_RESET, slave, fileName, (int) bytes_sent, progress);
          fflush(stdout);  
        }
        
    } else 
        printf("Slave[%d][error]: " ANSI_COLOR_RED "FAILED TO OPEN %s" ANSI_COLOR_RESET "\n", slave, fileName);

    fclose(fp);
    send(connSock, &fileInfo[1], sizeof(uint32_t), 0);
    printf(ANSI_COLOR_GREEN " [COMPLETED]\n" ANSI_COLOR_RESET);
}