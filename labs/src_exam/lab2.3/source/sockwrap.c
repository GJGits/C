...
/**
 * Setta il numero di porta ed in caso di errore
 * restituisce -1.
 */
int parsePort(const char *port, uint16_t *uPort)
{
	...
	for (int i = 0; i < lenght; i++)
	{
		...
		{
			printf("Server[port]: " ANSI_COLOR_RED "PORT IS NOT A NUMBER: [%s]" ANSI_COLOR_RESET "\n", port);
			return -1;
		}
	}
	// check if valid port
	...
	*uPort = !(longPort >= 1 && longPort <= 65535) ? 0 : (uint16_t)longPort;
	if (*uPort == 0)
	{
		printf("Server[port]: " ANSI_COLOR_RED "PORT IS OUT OF RANGE: [%s]" ANSI_COLOR_RESET "\n", port);
		return -1;
	}
