...
void doUdpClient(const char *address, const char *c_port) {
    ...
    setAddress(address, &server_address);
    printf("Server[address]: " ANSI_COLOR_GREEN "%s" ANSI_COLOR_RESET "\n", address);
    parsePort(c_port, &port);
    server_address.sin_family = AF_INET;
    server_address.sin_port = htons(port);
    sockfd = Socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    doUdpSend(sockfd, &server_address);
    doUdpReceive(sockfd, &server_address);
}

void doUdpSend(int sockfd, struct sockaddr_in *server_address) {
    socklen_t addr_len = sizeof(struct sockaddr_in);
    uint32_t send_buf = htonl(time(NULL));
    ssize_t sent = Sendto(sockfd, &send_buf, sizeof(send_buf), 
...
void doUdpReceive(int sockfd, struct sockaddr_in *server_address) {
    ...
    Recvfrom(sockfd, recv_buffer, (2 * sizeof(uint32_t)), 
            0, (struct sockaddr *) server_address, &addr_len);
    printf("one: %d, two: %d\n", (int)recv_buffer[0], (int)recv_buffer[1]);
}
