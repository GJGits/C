#!/bin/bash
# Script for testing socket final test 18-02-2020
...
function testClientServerInteraction
{
    ...
    runClient "./$2" "$client_dir" "127.0.0.1" "$server_port" "$SSIZE1" "$fname"
    ...
    # TEST the requested file is present in the client dir and is identical to original one starting from the offset
    # TEST 1.2/2.1/3.2
    testFilesEqualityOffset "$server_dir" "$client_dir" "$fname" "$SSIZE1"
...
#*********************************Test file present and correct from offset**********************************
# Check the expected file is present in the client directory and equals the content of the original file starting
...
# $2: the client directory
# $3: the name of the file to test
# $4: the offset on the original file
#
function testFilesEqualityOffset
{
    test_number=$(($test_number + 1))
    echo -e "\n\n[TEST $test_suite.$test_number] Checking the transferred file exists in client directory and its content is correct ..."
    local tname="TEST_$test_suite$test_number"
    local tname+="_PASSED"

    rm -f "$1/$OFFSET_FILE"
    dd iflag=skip_bytes bs=512k skip="$4" if="$1/$3" of="$1/$OFFSET_FILE" 2>&1 &> /dev/null
    cp "$2/$3" "$2/$OFFSET_FILE"

    fequality=$(getFileEquality "$1" "$2" "$OFFSET_FILE")
    if [[ $fequality == "0" ]] ; then
        echo -e "\n\t[++TEST $test_suite.$test_number PASSED++] Ok!"
        eval ${tname}=true
        result=passed
        ...
        echo -e "\n\t[--TEST $test_suite.$test_number FAILED--] File content is not as expected."
        eval ${tname}=false
        result=failed
    fi
}
...
if [ "$SERVER2_IS_PRESENT" = true ] ; then
    echo -e "\n\n*** PART 3: TESTS ON THE STUDENT'S SERVER2 WITH A REFERENCE CLIENT *************"
    test_suite=3
    test_number=0
    client_run=0
    ...
    if [[ $TEST_03_PASSED == true ]] ; then
        ...
        testClientServerInteraction "$SERVER2" "$TESTCLIENT1" false
...
if [[ $TEST_12_PASSED == true ]]; then
	...
	((nmessages++))
	echo -e "\n \t The student's server failed to deliver the file content as expected"
fi
...
if [[ $TEST_21_PASSED == true ]]; then
	...
	((nmessages++))
	echo -e "\n \t The interaction between the student's client and server did not work correctly (file is not correct)"
fi
...
if [[ $TEST_31_PASSED == true ]]; then
	...
	((nmessages++))
	echo -e "\n \t The interaction between the reference client and the server2 did not work correctly"
fi
...
if [[ $TEST_32_PASSED == true ]]; then
	...
	((nmessages++))
	echo -e "\n \t The student's server2 failed to deliver the file content as expected"
fi
